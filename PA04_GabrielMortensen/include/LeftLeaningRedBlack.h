...
public:
	...
	LLTB_t* NewNode(void);
    // Altered Datatype return  
	LLTB_t* LookUp(const std::uint32_t value);
    // ~~~Added Two below~~~~
       void GetNodeInfo (std::uint32_t key);
       LLTB_t* Parent_Info (std::uint32_t key);
    // ~~~~~~~~~~~~~~~~~~~~~~
	bool Insert(VoidRef_t ref, bool Right_Left);
	LLTB_t* InsertRec(LLTB_t *pNode, VoidRef_t ref, bool Right_Left);
