...
//
LLTB_t* LeftLeaningRedBlack::LookUp(std::uint32_t key)
{
...
//	Parent_Info()
//
//	Recursivley calls until it finds child
//	then returns one node up which is the parent 
// Idea 2... Finding the parent function 
// I think this might work because let's say you have
//
//                     xx
//		     /    \
//  		    y      e
...
// 		  z   w  g 
// 
// Use same method as LookUp to find child but then return parent 

LLTB_t* LeftLeaningRedBlack::Parent_Info (std::uint32_t key)
{  
     LLTB_t *pNode = m_pRoot; 
     LLTB_t *Parent = NULL;
...
// 		  z   w  g 
// 
//	Uses LookUp function to get child data (example g)
//	After outputing information about child 
//      it calls Parent_Info function which gets parent node (example e)

void LeftLeaningRedBlack::GetNodeInfo (std::uint32_t key)
{
 // Variables 
  bool FoundKid = 0;
 // Reobtain the just inserted Node and it's parent   
  LLTB_t *ChildNode = LookUp(key);
  LLTB_t *ParentNode = Parent_Info(ChildNode->Ref.Key);

 // if can't find child then do nothing 
  if (ChildNode != NULL)
  {
   // otherwise output child info 
    //system("Color 01");  
    std::cout << "\e[0;92m Inserted: " << "\e[0;37m" << ChildNode->Ref.Key; 
    if ( ChildNode->IsRed == TRUE )
      std::cout << " & Color is: \e[0;91m Red. \e[0;37m\n";
    else 
      std::cout << " & Color is: \e[0;93m Black. \e[0;37m\n";
   
   // And output parent info if it exists
    if (ChildNode != m_pRoot)
    {
        std::cout << "\e[0;95m Parent is: " << "\e[0;37m" << ParentNode->Ref.Key;
 	if ( ParentNode->IsRed == TRUE )
	      std::cout << " & Color is: \e[0;91m Red. \e[0;37m\n";
    	else 
      	      std::cout << " & Color is: \e[0;93m Black. \e[0;37m\n";
    }   
   // Otherwise there is only 1 child in tree thus the root 
    else 
      std::cout << "\e[0;37m " << ChildNode->Ref.Key << "\e[0;96m" << " is the root.\n";
  }
  else
...
//	Insert()
//
bool LeftLeaningRedBlack::Insert(VoidRef_t ref, bool Right_Left)
{
        bool Insert_Check = 0; 
	m_pRoot = InsertRec(m_pRoot, ref, Right_Left);
...
//
LLTB_t* LeftLeaningRedBlack::InsertRec(LLTB_t* pNode, VoidRef_t ref, bool Right_Left)
{
	// Special case for inserting a leaf.  Just return the pointer;
	// the caller will insert the new node into the parent node.
        // This will be the most recently inserted Node 
	if (NULL == pNode) {
		pNode      = NewNode();
		pNode->Ref = ref;
		return pNode;
	...
	//
		...
		if (ref.Key < pNode->Ref.Key) {
			pNode->pLeft = InsertRec(pNode->pLeft, ref, Right_Left);
		}
		else {
			pNode->pRight = InsertRec(pNode->pRight, ref, Right_Left);
		}
